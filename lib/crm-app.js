...
const { draftCampaignFromPrompt } = require('./ai');
...
function createCrmApp(options = {}) {
  ...
  async function routeApi(req, res, pathname) {
    try {
      ...
      if (req.method === 'POST' && pathname === '/api/ai/draft') {
        const payload = await readBody(req);
        return sendJson(res, 200, draftCampaignFromPrompt(state, payload.prompt || ''));
      }
      ...
      if (req.method === 'POST' && pathname.match(/^\/api\/campaigns\/[^/]+\/send$/)) {
        ...
        if (!channelResponse.ok) {
          ...
          persist();
          return sendJson(res, 502, { error: 'Channel service unavailable' });
        }
  ...
  return { server, state, applyReceipt };
}
