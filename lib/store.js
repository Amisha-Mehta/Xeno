...
function createCustomer(seed) {
  return {
    ...
    source: seed.source || 'Admin Created',
    address: seed.address || '',
    loyaltyPoints: seed.loyaltyPoints || (seed.loyaltyTier === 'gold' ? 350 : seed.loyaltyTier === 'silver' ? 150 : 50),
    lastActive: seed.lastActive || isoDaysAgo(seed.customerAgeDays),
    attributes: {
      gender: seed.gender,
      region: seed.region,
...
function evaluateSegment(segment, customer, state) {
  ...
  const requestedChannels = Array.isArray(segment.rules.channel)
    ? segment.rules.channel
    : segment.rules.preferredChannel
      ? [segment.rules.preferredChannel]
      : null;
  const channelMatch = requestedChannels ? requestedChannels.includes(preferredChannel) : true;
...
function createCampaignFromAudience(state, payload) {
  ...
  const recipientIds = Array.isArray(payload.recipientIds) ? payload.recipientIds : null;
  const audience = recipientIds
    ? state.customers.filter((customer) => recipientIds.includes(customer.id))
    : segment
...
function enrichCampaign(state, campaign) {
  const recipientAudience = Array.isArray(campaign.recipientIds)
  ...
  return {
    ...campaign,
    metrics: computeCampaignMetrics(state, campaign),
    audience: campaign.segmentId
      ? getAudienceMembers(state, getSegmentById(state, campaign.segmentId) || { rules: { kind: 'unknown' } })
      : recipientAudience
  };
}

function computeCustomerAiMetrics(state, customerId) {
  const customer = state.customers.find((c) => c.id === customerId);
  if (!customer) return null;

  const metrics = getOrderMetrics(state, customerId);
  const comms = state.communications.filter((c) => c.customerId === customerId);

  // LTV is metrics.totalSpend
  const ltv = metrics.totalSpend || 0;

  // Engagement Score
  // Base score 10. +10 per sent, +20 per delivered/opened/read, +40 per clicked. Max 100.
  let engagementScore = 10;
  comms.forEach((c) => {
    if (c.status === 'clicked') engagementScore += 40;
    else if (['read', 'opened', 'delivered'].includes(c.status)) engagementScore += 20;
  ...
  // Max 100. 
  // Opted-in: +20
  // Recent purchase (<= 45 days): +40; (<= 120 days): +20
  // Engagement: +40 if engagementScore > 50, +20 if engagementScore > 20
  let healthScore = 20;
  if (customer.optedIn) healthScore += 20;
  if (metrics.daysSinceLastOrder <= 45) healthScore += 40;
  else if (metrics.daysSinceLastOrder <= 120) healthScore += 20;
  if (engagementScore > 50) healthScore += 40;
  else if (engagementScore > 20) healthScore += 20;
  healthScore = Math.min(100, healthScore);

  // Churn Risk
  // Churn risk increases with days since last order.
  // E.g. if > 180 days: 90%. If > 90 days: 60%. If > 30 days: 30%. Else: 10%.
  // Also adjusted by opt-in status.
  let churnRisk = 10;
  if (metrics.daysSinceLastOrder > 180) churnRisk = 90;
  else if (metrics.daysSinceLastOrder > 90) churnRisk = 60;
  else if (metrics.daysSinceLastOrder > 30) churnRisk = 30;
  if (!customer.optedIn) churnRisk = Math.min(100, churnRisk + 15);

  // High-Value Customer Identification
  const isHighValue = ltv >= 8000 || customer.attributes.loyaltyTier === 'gold';

  // Next Best Action
  let nextBestAction = 'Recommend Trending Arrivals';
  if (!customer.optedIn) {
    nextBestAction = 'Request Opt-In via Email';
  } else if (churnRisk >= 60) {
    nextBestAction = 'Send 15% Win-back Discount';
  } else if (isHighValue && metrics.daysSinceLastOrder > 30) {
    nextBestAction = 'Invite to VIP Early Access';
  } else if (metrics.orderCount === 0) {
    nextBestAction = 'Send Onboarding Welcome Offer';
  } else if (metrics.daysSinceLastOrder <= 15) {
    nextBestAction = 'Request Product Review Feedback';
  }

  // Recommended Campaign
  let recommendedCampaign = 'Monsoon Layering Push';
  if (churnRisk >= 60) {
    recommendedCampaign = 'VIP Winback Promo';
  } else if (customer.preferredChannel === 'whatsapp') {
    recommendedCampaign = 'WhatsApp Special Collection';
  } else {
    recommendedCampaign = 'Weekly Newsletter Drop';
  }
