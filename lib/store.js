...
function createSeedState() {
  ...
  const segments = [
    ...
    },
    {
      ...
      type: 'ai',
      rules: { kind: 'lapsed', minDaysSinceOrder: 90, maxDaysSinceOrder: 365, minOrders: 2 },
      estimatedCount: 2,
...
function evaluateSegment(segment, customer, state) {
  ...
  switch (segment.rules.kind) {
    case 'vip_recent':
      return optInOk && channelMatch && tags.includes('VIP') && metrics.orderCount >= (segment.rules.minOrders || 2) && metrics.daysSinceLastOrder <= (segment.rules.maxDaysSinceOrder || 45);
    case 'lapsed':
      return optInOk && metrics.orderCount >= (segment.rules.minOrders || 2) && metrics.daysSinceLastOrder >= (segment.rules.minDaysSinceOrder || 90) && metrics.daysSinceLastOrder <= (segment.rules.maxDaysSinceOrder || 365);
    case 'channel_preference':
      return optInOk && channelMatch;
    case 'high_value':
      return optInOk && metrics.totalSpend >= (segment.rules.minSpend || 8000);
    case 'recent_buyer':
      return optInOk && metrics.daysSinceLastOrder <= (segment.rules.maxDaysSinceOrder || 30);
    case 'first_time':
      return optInOk && metrics.orderCount === 1;
    default:
      return optInOk && channelMatch;
  }
}
