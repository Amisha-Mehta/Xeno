...
function updateLogStatus(communicationId, status, source = 'simulation') {
  const log = messageLogs.find(l => l.communicationId === communicationId);
  if (log) {
    log.status = status;
    log.timestamp = new Date().toISOString();
    log.events.push({ type: status, timestamp: log.timestamp, source });
  }
}
  ...
  async function route(req, res, pathname) {
    try {
      ...
      if (req.method === 'POST' && pathname === '/send') {
        ...
        if (!payloadForceSimulation && !forceSimulation && whatsapp.isConfigured() && campaign.channel === 'whatsapp') {
          ...
          for (const comm of communications) {
            ...
            if (result.success) {
              ...
              setTimeout(() => {
                updateLogStatus(comm.id, 'failed', 'system');
        ...
        // If Email is configured AND campaign channel is email, send real emails
        console.log(`[channel] Email check: isConfigured=${email.isConfigured()}, channel=${campaign.channel}, forceSimulation=${forceSimulation}, payloadForceSimulation=${payloadForceSimulation}`);
        if (!payloadForceSimulation && !forceSimulation && email.isConfigured() && campaign.channel === 'email') {
          console.log(`[channel] Sending ${communications.length} real Emails via ${email.PROVIDER}`);

          const results = [];
          for (const comm of communications) {
            const emailAddr = comm.customer?.email;
            const subject = campaign.subject || 'Xeno Campaign';
            const message = comm.message || '';
            if (!emailAddr) {
              results.push({ communicationId: comm.id, success: false, error: 'No email address' });
              logSentMessage({
                communicationId: comm.id,
                campaignId: campaign.id,
                campaignName: campaign.name,
            ...
            const result = await email.sendMessage(emailAddr, subject, message);
            console.log(`[channel] Send result for ${emailAddr}:`, result);
            results.push({ communicationId: comm.id, ...result });
            ...
            logSentMessage({
              ...
              callbackUrl,
              externalId: result.messageId || '',
              recipientPhone: emailAddr,
              events: [{ type: result.success ? 'delivered' : 'failed', timestamp: new Date().toISOString(), source: 'system' }]
            });

            // Send immediate receipt back to CRM callback URL
            const receipt = {
              id: uid('evt'),
              campaignId: campaign.id,
              communicationId: comm.id,
              customerId: comm.customer.id,
              channel: campaign.channel,
              type: result.success ? 'delivered' : 'failed',
              timestamp: new Date().toISOString(),
              meta: { provider: result.provider, error: result.error, real: true }
            };
            setTimeout(() => {
              updateLogStatus(comm.id, result.success ? 'delivered' : 'failed');
        ...
        // If SMS is configured AND campaign channel is sms, send real SMS
        console.log(`[channel] SMS check: isConfigured=${sms.isConfigured()}, channel=${campaign.channel}, forceSimulation=${forceSimulation}, payloadForceSimulation=${payloadForceSimulation}`);
        if (!payloadForceSimulation && !forceSimulation && sms.isConfigured() && campaign.channel === 'sms') {
          console.log(`[channel] Sending ${communications.length} real SMS via ${sms.PROVIDER}`);
          ...
          for (const comm of communications) {
            ...
            const result = await sms.sendMessage(phone, message);
            console.log(`[channel] Send result for ${phone}:`, result);
            results.push({ communicationId: comm.id, ...result });
