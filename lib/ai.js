...
async function aiChat(state, userMessage) {
  ...
  try {
    ...
    if (!response.ok) {
      const providerError = await readApiError(response);
      return {
        reply: `AI service is configured, but the provider rejected the request: ${providerError}`,
        mode: 'error'
      };
    }
    ...
    return {
      reply: `AI service is configured, but the request could not be completed: ${error.message}. The offline draft engine is still available in the Command Center.`,
      mode: 'error'
    };
  }
}
