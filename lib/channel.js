...
function maybeScheduleDuplicate(onReceipt, payload, delay) {
  if (Math.random() < 0.12) {
    scheduleReceipt(onReceipt, payload, delay + randomBetween(250, 1100));
  }
}
...
function sendCampaign({ campaign, communications, onReceipt, renderMessage }) {
  ...
  communications.forEach((communication, index) => {
    ...
    maybeScheduleDuplicate(onReceipt, deliveryEvent, baseDelay + randomBetween(1600, 2600));
    ...
    if (Math.random() < profile.open) {
      ...
      scheduleReceipt(onReceipt, openEvent, baseDelay + randomBetween(900, 3200));
    }
    ...
    if (Math.random() < profile.read) {
      ...
      scheduleReceipt(onReceipt, readEvent, baseDelay + randomBetween(1200, 4600));
    }
    ...
    if (Math.random() < profile.click) {
      ...
      scheduleReceipt(onReceipt, clickEvent, baseDelay + randomBetween(1800, 6400));
      maybeScheduleDuplicate(onReceipt, clickEvent, baseDelay + randomBetween(6400, 7600));
